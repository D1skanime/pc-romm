...
def _source_manifest(path: Path) -> tuple[tuple[object, ...], ...]:
...
def test_health_snapshot_is_live_and_does_not_mutate_model_or_source(
    tmp_path: Path,
) -> None:
    (tmp_path / "game.txt").write_text("archive", encoding="utf-8")
    root = _root(tmp_path)
    model_before = dict(root.__dict__)
    source_before = _source_manifest(tmp_path)
    ...
    assert (health.reachable, health.readable, health.non_writable) == (
        True,
        True,
        True,
    )
    ...
    assert root.__dict__ == model_before
    assert _source_manifest(tmp_path) == source_before


def test_browse_returns_globally_sorted_minimal_bounded_pages(tmp_path: Path) -> None:
    for name in ["Zulu", "München", "Alpha", "日本語"]:
        (tmp_path / name).mkdir()
    (tmp_path / "file.txt").write_text("not a directory", encoding="utf-8")
    root = _root(tmp_path)
    before = _source_manifest(tmp_path)
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        first = browse_storage_directories(root, "", limit=2)
        second = browse_storage_directories(root, "", limit=2, cursor=first.next_cursor)
    assert [
        (entry.name, entry.relative_path, entry.navigable) for entry in first.entries
    ] == [("Alpha", "Alpha", True), ("München", "München", True)]
    assert [entry.name for entry in second.entries] == ["Zulu", "日本語"]
    assert second.next_cursor is None
    assert set(vars(first.entries[0])) == {"name", "relative_path", "navigable"}
    assert _source_manifest(tmp_path) == before


def test_browse_nested_parent_and_cursor_are_query_bound(tmp_path: Path) -> None:
    (tmp_path / "parent" / "A").mkdir(parents=True)
    (tmp_path / "parent" / "B").mkdir()
    other = _root(tmp_path)
    other.id = 18
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        page = browse_storage_directories(_root(tmp_path), "parent", limit=1)
        assert page.entries[0].relative_path == "parent/A"
        with pytest.raises(InvalidStorageCursorError):
            browse_storage_directories(
                other, "parent", limit=1, cursor=page.next_cursor
            )
        with pytest.raises(InvalidStorageCursorError):
            browse_storage_directories(
                _root(tmp_path), "", limit=1, cursor=page.next_cursor
            )


@pytest.mark.parametrize("cursor", ["not-base64", "x" * 3000])
def test_browse_rejects_malformed_or_oversized_cursor(
    tmp_path: Path, cursor: str
) -> None:
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        with pytest.raises(InvalidStorageCursorError):
            browse_storage_directories(_root(tmp_path), "", cursor=cursor)


@pytest.mark.parametrize("parent", ["..", "/etc", r"C:\Games", r"server\share"])
def test_browse_rejects_unsafe_parent_syntax(tmp_path: Path, parent: str) -> None:
    with pytest.raises(InvalidRelativePathError):
        browse_storage_directories(_root(tmp_path), parent)


def test_browse_rejects_symlink_entries_and_safe_filesystem_failures(
    tmp_path: Path,
) -> None:
    ...
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        with pytest.raises(UnsafeSymlinkError):
            browse_storage_directories(root, "")
        with patch(
            "handler.filesystem.storage_resolver.os.scandir",
            side_effect=PermissionError("/secret/raw/path"),
        ):
            with pytest.raises(SafeStorageFilesystemError) as exc_info:
                browse_storage_directories(root, "")
    assert "/secret/raw/path" not in str(exc_info.value)
    assert len(str(exc_info.value)) <= 200


def test_browse_scans_all_entries_before_selecting_page_and_enforces_ceiling(
    tmp_path: Path,
) -> None:
    for name in ["Zulu", "Yankee", "Alpha"]:
        (tmp_path / name).mkdir()
    root = _root(tmp_path)
    with patch(
        "handler.filesystem.storage_resolver.os.access", _permission_observer
    ), patch("handler.filesystem.storage_resolver.MAX_DIRECTORY_SCAN_ENTRIES", 2):
        with pytest.raises(StorageScanLimitError):
            browse_storage_directories(root, "", limit=1)
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        page = browse_storage_directories(root, "", limit=1)
    assert page.entries[0].name == "Alpha"
...
def test_storage_observation_has_no_mutation_primitives(
    ...
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        {
            "health": check_storage_root_health,
            "root": resolve_storage_root,
            "target": lambda value: resolve_directory(value, "games"),
        }[operation](root)
...
def test_health_reports_missing_unreadable_and_writable_roots(tmp_path: Path) -> None:
    ...
    assert (missing.reachable, missing.readable, missing.non_writable) == (
        False,
        False,
        None,
    )
...
def test_internal_root_resolution_is_the_only_empty_path_contract(
    tmp_path: Path,
) -> None:
    root = _root(tmp_path)
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
...
def test_nested_target_resolves_inside_canonical_root(tmp_path: Path) -> None:
    ...
    with patch("handler.filesystem.storage_resolver.os.access", _permission_observer):
        assert (
            resolve_directory(_root(tmp_path), "Sony.PlayStation-2/M\u00fcnchen")
            == target.resolve()
        )


@pytest.mark.parametrize(
    ("position", "link_target"),
    [
        ("root", "absolute-in-root"),
        ("root", "relative-in-root"),
        ("first", "absolute-in-root"),
        ("first", "relative-escape"),
        ("middle", "relative-in-root"),
        ("leaf", "absolute-escape"),
        ("leaf", "broken"),
        ("leaf", "loop"),
    ],
)
def test_symlink_at_every_component_and_target_form_is_rejected(
    tmp_path: Path, position: str, link_target: str
) -> None:
    ...
    if position == "root":
        ...
        root_path.symlink_to(
            real_root if link_target == "absolute-in-root" else Path("library"),
            target_is_directory=True,
        )
    else:
        link = {
            "first": real_root / "first",
            "middle": real_root / "first" / "middle",
            "leaf": real_target,
        }[position]
        if link.is_dir():
            shutil.rmtree(link)
        destinations = {
            "absolute-in-root": real_target,
            "relative-in-root": Path("../middle/leaf"),
            "relative-escape": Path("../../library-old"),
...
def test_canonical_sibling_prefix_is_not_containment(tmp_path: Path) -> None:
    ...
    with patch(
        ...
        "handler.filesystem.storage_resolver._strict_resolve_target",
        return_value=sibling.resolve(),
    ):
