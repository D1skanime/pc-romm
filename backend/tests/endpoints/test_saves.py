...
class TestDetachedSaveLifecycle:
    ...
    def test_detached_save_lists_gets_downloads_and_masks_hidden_platform(
        self,
        mock_validate_path,
        client,
        access_token: str,
        ...
        listing = client.get("/api/saves", headers=_auth(access_token))
        filtered = client.get(
            f"/api/saves?platform_id={platform.id}", headers=_auth(access_token)
        )
        detail = client.get(f"/api/saves/{save.id}", headers=_auth(access_token))
        assert listing.status_code == status.HTTP_200_OK
        assert filtered.status_code == status.HTTP_200_OK
        assert detail.status_code == status.HTTP_200_OK
        listed = next(item for item in listing.json() if item["id"] == save.id)
        assert listed["rom_id"] is None
        assert isinstance(listed["retained_catalog_id"], int)
        assert [item["id"] for item in filtered.json()] == [save.id]
        assert detail.json()["retained_catalog_id"] == listed["retained_catalog_id"]

        other_detail = client.get(
            f"/api/saves/{save.id}", headers=_auth(viewer_access_token)
        )
        assert other_detail.status_code == status.HTTP_404_NOT_FOUND

        test_file = tmp_path / "detached.sav"
        test_file.write_bytes(b"DETACHED_SAVE")
        mock_validate_path.return_value = test_file
        owner_content = client.get(
            f"/api/saves/{save.id}/content", headers=_auth(access_token)
        )
        assert owner_content.status_code == status.HTTP_200_OK
        assert owner_content.content == b"DETACHED_SAVE"
        ...
        public_content = client.get(
            f"/api/saves/{save.id}/content", headers=_auth(viewer_access_token)
        )
        ...
        assert masked.json() == {"detail": f"Save with ID {save.id} not found"}
    ...
    def test_detached_save_updates_owned_content_conflicts_on_screenshot_and_deletes(
        self,
        mock_scan_save,
        mock_write_file,
        mock_remove_file,
        ...
        _detach_rom(client, access_token, rom)
        retained = db_save_handler.get_save(user_id=admin_user.id, id=save.id)
        ...
        with mock.patch(
            "endpoints.saves.authorize_api_storage_operation",
        ...
        mock_write_file.assert_awaited_once_with(
            file=mock.ANY, path=save.file_path, filename=save.file_name
        )
        retained = db_save_handler.get_save(user_id=admin_user.id, id=save.id)
        assert retained is not None
        ...
        unchanged = db_save_handler.get_save(user_id=admin_user.id, id=save.id)
        ...
        assert db_save_handler.get_save(user_id=admin_user.id, id=save.id) is None
