...
use romm_download_core::{
    ...
    HttpResponse, HttpTransport, HttpTransportError, TransferOutcome, VecSink,
};
...
fn fetches_only_an_origin_bound_manifest_before_accepting_a_root_handle() {
    ...
    let queued = engine
        .queue_manifest(origin, MANIFEST_ID.parse().unwrap(), handle)
        .unwrap();

    assert_eq!(queued.manifest.members[0].size, 3);
    assert_eq!(
        transport.requests[0].url,
        format!("https://romm.example:8443/api/download-manifests/{MANIFEST_ID}")
    );
    assert_eq!(
        transport.requests[0].headers.get("Authorization"),
        Some(&"Bearer device-token".to_owned())
    );
...
fn invalid_remote_states_do_not_create_a_job_or_accept_cross_origin_data() {
    ...
    for (status, expected) in [
        (401, DownloadError::AuthRequired),
        (403, DownloadError::AuthRequired),
        (410, DownloadError::ManifestStale),
        (409, DownloadError::SourceChanged),
    ] {
        let mut transport = MockTransport::with(response(status, b"{}"));
        let mut engine = DownloadEngine::new(&mut transport, &roots, "secret");
        assert_eq!(
            engine
                .queue_manifest(origin.clone(), MANIFEST_ID.parse().unwrap(), handle)
                .unwrap_err(),
            expected
        );
    }
...
fn malformed_manifests_and_expired_handles_create_no_local_state() {
    ...
    assert_eq!(
        engine
            .queue_manifest(origin, MANIFEST_ID.parse().unwrap(), handle)
            .unwrap_err(),
        DownloadError::InvalidDestinationRoot
    );
    assert!(fs::read_dir(&local_root).unwrap().next().is_none());
}
...
fn fresh_and_resume_protocols_require_exact_status_range_snapshot_and_lengths() {
    ...
    fresh
        .headers
        .insert("Content-Length".to_owned(), "3".to_owned());
    engine.replace_transport(MockTransport::with(fresh));
    let mut sink = VecSink::default();
    assert_eq!(
        engine.transfer_member(&member, 0, &mut sink).unwrap(),
        TransferOutcome::Downloaded { bytes: 3 }
    );
    ...
    resumed
        .headers
        .insert("Content-Length".to_owned(), "1".to_owned());
    resumed
        .headers
        .insert("Content-Range".to_owned(), "bytes 2-2/3".to_owned());
    engine.replace_transport(MockTransport::with(resumed));
    let mut sink = VecSink::default();
    assert_eq!(
        engine.transfer_member(&member, 2, &mut sink).unwrap(),
        TransferOutcome::Downloaded { bytes: 3 }
    );
    assert_eq!(
        engine.last_request().unwrap().headers.get("Range"),
        Some(&"bytes=2-".to_owned())
    );
    assert_eq!(
        engine.last_request().unwrap().headers.get("If-Match"),
    ...
    assert_eq!(
        engine
            .transfer_member(&member, 2, &mut VecSink::default())
            .unwrap_err(),
        DownloadError::InvalidResponse
    );
}
...
fn classifies_416_and_large_u64_metadata_without_allocating_parts() {
    ...
    for size in [
        ...
        assert_eq!(
            engine
                .transfer_member(&queued.manifest.members[0], size, &mut VecSink::default())
                .unwrap(),
            TransferOutcome::VerificationCandidate
        );
        assert_eq!(
            engine
                .transfer_member(
                    &queued.manifest.members[0],
                    size + 1,
                    &mut VecSink::default()
                )
                .unwrap_err(),
            DownloadError::CorruptLocalState
        );
    }
}
